...
# DATOS DEL SERVICIO
# =====================================================
SERVICE_NAME = "servicio_procesador_imagen_modelo_local"
...
def image_to_base64(img):
    ...
    if not ok:
        raise RuntimeError("Error codificando imagen")
...
def run_service():
    ...
    print(f"⚡ {SERVICE_NAME} iniciado en {cfg.get('host', '0.0.0.0')}:{cfg.get('service_port', 5004)}")

    app.run(
        host=cfg.get("host", "0.0.0.0"),
        port=cfg.get("service_port", 5004),
        threaded=True,
        debug=False
    )
